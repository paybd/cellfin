...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 bg-[#196642] text-white shadow-md">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 shrink-0">
            <Image src="/assets/mcash.png" alt="Cellfin Remit" width={40} height={40} className="object-contain" />
            <span className="text-lg font-semibold hidden sm:block">Cellfin Remit</span>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm text-white/90">
            ...
            <Link href="/cellfin_remit_reseller.apk" className="rounded-lg bg-white text-[#196642] px-4 py-2 text-sm font-semibold hover:bg-white/90 transition">ডাউনলোড</Link>
          </nav>
        ...
        {isMenuOpen && (
          <div className="md:hidden pb-4 border-t border-white/20">
            <nav className="flex flex-col gap-2 pt-4">
              ...
              <Link href="/cellfin_remit_reseller.apk" onClick={closeMenu} className="px-4 py-2 text-sm text-center rounded-lg bg-white text-[#196642] font-semibold hover:bg-white/90 transition">ডাউনলোড</Link>
            </nav>
          </div>
        )}
      </div>
